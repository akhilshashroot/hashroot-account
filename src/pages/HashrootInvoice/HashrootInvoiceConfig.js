...
import HashrootInvoiceDetails from './HashrootInvoiceDetails';
import HashrootInvoiceAddEdit from './HashrootInvoiceAdd';
import HashrootInvoiceDelete from './HashrootInvoiceDelete';
import { toast, ToastContainer, Zoom } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
// import "../../assets/scss/toastr.scss"
import { connect } from 'react-redux';
import {
    getHashrootInvoiceList,
...
const HashrootInvoiceConfig = (props) => {
    ...
    useEffect(() => {
        if (props.hashrootinvoice && !props.hashrootinvoice.hashrootinvoice) {
            props.getHashrootInvoiceList();
        }
    }, []);
                ...
                <CardBody>
                    <React.Fragment>
                        <Row>
                            <Col className="text-right mb-1">
                                <Button color="danger" className="mb-2" onClick={() => handleAddEditModal()}>
                                    <i className="mdi mdi-plus-circle mr-2"></i> Add New HashrootInvoice
                                </Button>
                            </Col>
                        </Row>
                        ...
                        {props.hashrootinvoice &&
                            props.hashrootinvoice.hashrootinvoice &&
                            props.hashrootinvoice.hashrootinvoice.data && (
                            <ToolkitProvider bootstrap4 keyField="id" data={props.hashrootinvoice &&
                                props.hashrootinvoice.hashrootinvoice &&
                                props.hashrootinvoice.hashrootinvoice.data} columns={columns} search columnToggle>
                                {(props) => (
                                    <React.Fragment>
                                        <Row>
                                            <Col md={4} className="d-flex justify-content-start flex-column">
                        ...
                        {toggleAddEditModal && (
                            <HashrootInvoiceAddEdit
                                ...
                                getHashrootInvoiceAdd={props.getHashrootInvoiceAdd}
                                getHashrootInvoiceUpdate={props.getHashrootInvoiceUpdate}
                                hashrootinvoice={props.hashrootinvoice}
                                getHashrootInvoiceList={props.getHashrootInvoiceList}
                            />
                        )}
                        ...
                        {toggleDeleteModal && (
                            <HashrootInvoiceDelete
                                ...
                                getHashrootInvoiceDelete={props.getHashrootInvoiceDelete}
                                data={hashrootinvoiceData}
                                records={
                                    props.hashrootinvoice &&
                                    props.hashrootinvoice.hashrootinvoice &&
...
const mapStateToProps = (state) => {
    return {
        hashrootinvoice: state.HashrootInvoice,
        auth : state.Auth
    };
};
