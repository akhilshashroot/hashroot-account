...
import HashrootInvoiceDetails from './HashrootpInvoiceDetails';
import HashrootInvoiceAddEdit from './HashrootpInvoiceAdd';
import HashrootInvoiceDelete from './HashrootpInvoiceDelete';
import { toast, ToastContainer, Zoom } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
// import "../../assets/scss/toastr.scss"
import { connect } from 'react-redux';
import {
    getHashrootpInvoiceList,
    getHashrootpInvoiceAdd,
    getHashrootpInvoiceUpdate,
    getHashrootpInvoiceDelete,
    getHashrootpCloneInvoice,
    downloadInvoice,
} from '../../redux/actions';
...
const HashrootInvoiceConfig = (props) => {
            ...
            formatter: (cell, row, rowIndex, formatExtraData) => (
                <div style={{ lineHeight: 'normal', margin: 0 }}>
                    <i
                        ...
                        title="Download"
                        onClick={() => downloadInvoice(row, rowIndex)}
                        id="download"></i>
    ...
    useEffect(() => {
        if (props.hashrootinvoice && !props.hashrootinvoice.hashrootpinvoice) {
            props.getHashrootpInvoiceList();
        }
        if (props?.hashrootinvoice?.hashrootpCloneinvoice) {
            props.getHashrootpInvoiceList();
        }
        if(props?.hashrootinvoice?.downloadInvoice?.pdf_url){
            window.open(props?.hashrootinvoice?.downloadInvoice?.pdf_url, '_blank');
        }
    }, [props?.hashrootinvoice?.hashrootpCloneinvoice,props?.hashrootinvoice?.downloadInvoice?.pdf_url]);
    ...
    const invoiceClone = (row, index) => {
        ...
        props.getHashrootpCloneInvoice(data);
    };
    const downloadInvoice = (row, index) => {
        let data = {
            id: row.invoice_id,
        };
        props.downloadInvoice(data);
    };
            ...
            <Card>
                <CardBody>
                    <React.Fragment>
                        <Row>
                            <Col className="text-right mb-1">
                                <Button color="danger" className="mb-2" onClick={() => handleAddEditModal()}>
                                    <i className="mdi mdi-plus-circle mr-2"></i> Add New Hashroot-P Invoice
                                </Button>
                            </Col>
                        </Row>
                        ...
                        {props.hashrootinvoice &&
                            props.hashrootinvoice.hashrootpinvoice &&
                            props.hashrootinvoice.hashrootpinvoice.data && (
                                <ToolkitProvider
                                    bootstrap4
                                    keyField="id"
                                    data={
                        ...
                        {toggleAddEditModal && (
                            <HashrootInvoiceAddEdit
                                ...
                                getHashrootInvoiceAdd={props.getHashrootpInvoiceAdd}
                                getHashrootInvoiceUpdate={props.getHashrootpInvoiceUpdate}
                                hashrootinvoice={props.hashrootinvoice}
                                getHashrootpInvoiceList={props.getHashrootpInvoiceList}
                            />
                        )}
                        ...
                        {toggleDeleteModal && (
                            <HashrootInvoiceDelete
                                ...
                                getHashrootInvoiceDelete={props.getHashrootpInvoiceDelete}
                                data={hashrootinvoiceData}
                                records={
                                    props.hashrootinvoice &&
                                    props.hashrootinvoice.hashrootinvoice &&
...
const mapStateToProps = (state) => {
    return {
        hashrootinvoice: state.Hashrootp,
        auth: state.Auth,
    };
};
