...
import HashrootInvoiceDetails from './HashrootssInvoiceDetails';
import HashrootInvoiceAddEdit from './HashrootssInvoiceAdd';
import HashrootInvoiceDelete from './HashrootssInvoiceDelete';
import { toast, ToastContainer, Zoom } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
// import "../../assets/scss/toastr.scss"
import { connect } from 'react-redux';
import {
    getHashrootssInvoiceList,
    getHashrootssInvoiceAdd,
    getHashrootssInvoiceUpdate,
    getHashrootssInvoiceDelete,
    getHashrootssCloneInvoice,
    downloadInvoicess,
} from '../../redux/actions';
...
const HashrootssInvoiceConfig = (props) => {
            ...
            formatter: (cell, row, rowIndex, formatExtraData) => (
                <div style={{ lineHeight: 'normal', margin: 0 }}>
                    <i
                        ...
                        title="Download"
                        onClick={() => downloadInvoicess(row, rowIndex)}
                        id="download"></i>
    ...
    useEffect(() => {
        if (props.hashrootinvoice && !props.hashrootinvoice.hashrootssinvoice) {
            props.getHashrootssInvoiceList();
        }
        if (props?.hashrootinvoice?.hashrootssCloneinvoice) {
            props.getHashrootssInvoiceList();
        }
        if(props?.hashrootinvoice?.downloadInvoicess?.pdf_url){
            window.open(props?.hashrootinvoice?.downloadInvoicess?.pdf_url, '_blank');
        }
    }, [props?.hashrootinvoice?.hashrootssCloneinvoice,props?.hashrootinvoice?.downloadInvoicess?.pdf_url]);
    ...
    const invoiceClone = (row, index) => {
        ...
        props.getHashrootssCloneInvoice(data);
    };
    const downloadInvoicess = (row, index) => {
        let data = {
            id: row.invoice_id,
        };
        props.downloadInvoicess(data);
    };
            ...
            <Card>
                <CardBody>
                    <React.Fragment>
                        <Row>
                            <Col className="text-right mb-1">
                                <Button color="danger" className="mb-2" onClick={() => handleAddEditModal()}>
                                    <i className="mdi mdi-plus-circle mr-2"></i> Add New Hashroot-SS Invoice
                                </Button>
                            </Col>
                        </Row>
                        ...
                        {props.hashrootinvoice &&
                            props.hashrootinvoice.hashrootssinvoice &&
                            props.hashrootinvoice.hashrootssinvoice.data && (
                                <ToolkitProvider
                                    bootstrap4
                                    keyField="id"
                                    data={
                        ...
                        {toggleAddEditModal && (
                            <HashrootInvoiceAddEdit
                                ...
                                getHashrootInvoiceAdd={props.getHashrootssInvoiceAdd}
                                getHashrootInvoiceUpdate={props.getHashrootssInvoiceUpdate}
                                hashrootinvoice={props.hashrootinvoice}
                                getHashrootssInvoiceList={props.getHashrootssInvoiceList}
                            />
                        )}
                        ...
                        {toggleDeleteModal && (
                            <HashrootInvoiceDelete
                                ...
                                getHashrootInvoiceDelete={props.getHashrootssInvoiceDelete}
                                data={hashrootinvoiceData}
                                records={
                                    props.hashrootinvoice &&
                                    props.hashrootinvoice.hashrootinvoice &&
