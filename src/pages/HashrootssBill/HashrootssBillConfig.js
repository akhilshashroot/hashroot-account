...
import HashrootBillDetails from './HashrootssBillDetails';
import HashrootBillAddEdit from './HashrootssBillAdd';
import HashrootBillDelete from './HashrootssBillDelete';
import { toast, ToastContainer, Zoom } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
// import "../../assets/scss/toastr.scss"
import { connect } from 'react-redux';
import {
    getHashrootssBillList,
    getHashrootssBillAdd,
    getHashrootssBillUpdate,
    getHashrootssBillDelete,
    getHashrootssCloneBill,
    downloadBill,
} from '../../redux/actions';
...
const HashrootssBillConfig = (props) => {
    ...
    const invoiceClone = (row, index) => {
        ...
        props.getHashrootssCloneBill(data);
    };
                    ...
                    <React.Fragment>
                        <Row>
                            <Col className="text-right mb-1">
                                <Button color="danger" className="mb-2" onClick={() => handleAddEditModal()}>
                                    <i className="mdi mdi-plus-circle mr-2"></i> Add New Hashroot-SS Bill
                                </Button>
                            </Col>
                        </Row>
                        ...
                        {toggleAddEditModal && (
                            <HashrootBillAddEdit
                                ...
                                getHashrootBillAdd={props.getHashrootssBillAdd}
                                getHashrootBillUpdate={props.getHashrootssBillUpdate}
                                hashrootbill={props.hashrootbill}
                                getHashrootssBillList={props.getHashrootssBillList}
                            />
                        )}
                        ...
                        {toggleDeleteModal && (
                            <HashrootBillDelete
                                ...
                                getHashrootBillDelete={props.getHashrootssBillDelete}
                                data={hashrootbillData}
                                records={
                                    props.hashrootbill &&
                                    props.hashrootbill.hashrootbill &&
...
const mapStateToProps = (state) => {
    return {
        hashrootbill: state.HashrootssBill,
        auth: state.Auth,
    };
};
