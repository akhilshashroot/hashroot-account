...
import HashrootBillDetails from './HashrootBillDetails';
import HashrootBillAddEdit from './HashrootBillAdd';
import HashrootBillDelete from './HashrootBillDelete';
...
import {
    getHashrootBillList,
    getHashrootBillAdd,
    getHashrootBillUpdate,
    getHashrootBillDelete,
    getHashrootCloneBill,
    downloadBill,
} from '../../redux/actions';
...
const HashrootBillConfig = (props) => {
    ...
    useEffect(() => {
        if (props.hashrootbill && !props.hashrootbill.hashrootinvoice) {
            props.getHashrootBillList();
        }
        if (props?.hashrootbill?.HashrootClonebill) {
            console.log("called clone");
            props.getHashrootBillList();
        }
        if (props?.hashrootbill?.downloadBill?.pdf_url) {
            window.open(props?.hashrootbill?.downloadBill?.pdf_url, '_blank');
        }
    }, [props?.hashrootbill?.HashrootClonebill, props?.hashrootbill?.downloadBill?.pdf_url]);
    ...
    const invoiceClone = (row, index) => {
        ...
        props.getHashrootCloneBill(data);
    };
                    ...
                    <React.Fragment>
                        <Row>
                            <Col className="text-right mb-1">
                                <Button color="danger" className="mb-2" onClick={() => handleAddEditModal()}>
                                    <i className="mdi mdi-plus-circle mr-2"></i> Add New Hashroot Bill
                                </Button>
                            </Col>
                        </Row>
                        ...
                        {toggleAddEditModal && (
                            <HashrootBillAddEdit
                                ...
                                getHashrootBillAdd={props.getHashrootBillAdd}
                                getHashrootBillUpdate={props.getHashrootBillUpdate}
                                hashrootbill={props.hashrootbill}
                                getHashrootBillList={props.getHashrootBillList}
                            />
                        )}
                        ...
                        {toggleDeleteModal && (
                            <HashrootBillDelete
                                ...
                                getHashrootBillDelete={props.getHashrootBillDelete}
                                data={hashrootbillData}
                                records={
                                    props.hashrootbill &&
                                    props.hashrootbill.hashrootbill &&
...
const mapStateToProps = (state) => {
    return {
        hashrootbill: state.HashrootBill,
        auth: state.Auth,
    };
};
